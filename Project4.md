...
#### Steps
1. ##### Install Nodejs
  * Provision Ubuntu 20.04 (or 22.04) instance in AWS
  ...
  * Add certificates and the NodeSource repository for a current Node.js LTS release (this tutorial originally used Node 12, which reached End-of-Life in April 2022 — always install an active LTS version):
```bash
...
![Project4pix1](https://user-images.githubusercontent.com/74002629/177740062-bfa83002-6ee8-431a-81d2-0b5517fd470d.PNG)
  ...
  * Verify the installed version: `node -v` (should print `v20.x.x`) — check [nodejs.org](https://nodejs.org) for the current LTS before you provision, since NodeSource setup scripts are updated regularly.
  
 ![Project4pix2](https://user-images.githubusercontent.com/74002629/177740077-2135389b-62ec-423f-9a16-95ca7a29ce73.PNG)
  
2. ##### Install MongoDB
  * The original version of this project pointed at MongoDB 3.4 from Ubuntu 14.04 ("trusty"), which has been End-of-Life since 2018, and used the deprecated `apt-key` method. Use the current MongoDB Community Edition repository and the modern signed-by keyring method instead:
```bash
# Import the MongoDB public GPG key into a dedicated keyring (not the deprecated apt-key keyring)
curl -fsSL https://pgp.mongodb.com/server-7.0.asc | sudo gpg --dearmor -o /usr/share/keyrings/mongodb-server-7.0.gpg

# Add the repository, matching it to your Ubuntu release codename (jammy = 22.04, focal = 20.04)
echo "deb [ arch=amd64,arm64 signed-by=/usr/share/keyrings/mongodb-server-7.0.gpg ] https://repo.mongodb.org/apt/ubuntu jammy/mongodb-org/7.0 multiverse" | sudo tee /etc/apt/sources.list.d/mongodb-org-7.0.list

sudo apt update
```
  * Install MongoDB with the following command: `sudo apt install -y mongodb-org`
  * Start and enable the service (package name changed from `mongodb` to `mongod` in modern MongoDB packages):
```bash
...
```
  ...
  * Verify the server is up and running: `sudo systemctl status mongod`
  
   ![Project4pix5](https://user-images.githubusercontent.com/74002629/177740147-d4fcae4f-a5e7-4d1a-86e2-7d480b1efd4b.PNG)

> 🔒 **Security consideration:** This lab binds MongoDB to localhost by default, which is correct for a single-server demo. In production, MongoDB should never be exposed directly to the internet — restrict it to a private subnet/security group, enable authentication (`security.authorization: enabled` in `/etc/mongod.conf`), and create role-scoped database users instead of relying on an open, unauthenticated instance.
