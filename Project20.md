...
3. Add the GPG key for the official Docker repository to your system (modern signed-by method — `apt-key` is deprecated and removed in Ubuntu 22.04+):
```bash
...
sudo apt update
```
> 💡 In production, prefer installing the `docker-ce`, `docker-ce-cli`, `containerd.io`, `docker-buildx-plugin`, and `docker-compose-plugin` packages together rather than the legacy `docker.io` package — this gets you BuildKit and Compose v2 (the `docker compose` subcommand) out of the box.
...
` sudo docker run --network tooling_app_network --name mysql-client -it --rm mysql mysql -h mysqlserverhost -u cynthia -p `
...
I solved it by going into the db_conn.php file and hardcoded my environment variable. This is not the recommended way of doing it but this was how I got mine to work.
...
6. Access your tooling site again: `http://<server-publicIP>:8085`
